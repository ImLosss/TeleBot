    ...
    if (req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          bot.sendMessage(config.ID_CHANNEL, text, { parse_mode: 'HTML', inline_keyboard: [[{ text: 'Dukung Kami', url: 'https://sociabuzz.com/dongworld/tribe' }]] }).catch(err => console.error('Failed to send message:', err));
        } catch (e) {
