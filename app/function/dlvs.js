...
async function dlvs_downloadVideo(bot, query, data) {
    ...
    if (hardsub) {
        cmd = `yt-dlp -f ${format_id}+bestaudio --remux-video ${ext} --write-sub --sub-langs ${lang} --sub-format ${ext_lang} --convert-subs srt -o "${outputTemplate}" "${url}" --no-warnings --no-call-home --no-check-certificate --ffmpeg-location /usr/bin/ffmpeg --cookies-from-browser firefox && ffmpeg -i "downloads/${id}.${ext}" -crf "27" -vf "subtitles=downloads/${id}.${lang}.srt:force_style='FontName=Arial,FontSize=${fontSize},PrimaryColour=&HFFFFFF&,Outline=${outline},MarginV=${y},Bold=1',drawtext=text='DongWorld':font=Verdana:fontsize=${wmSize}:fontcolor=white@0.5:x=15:y=15, drawtext=text='Join Channel t.me/dongworld agar tidak ketinggalan update':font='Arial-Bold':fontsize=${wmSize}:fontcolor=white:borderw=${outline}:x=(w-text_w)/2:y=h-50:enable='lt(t,25)'" -c:a copy "downloads/${id}_hardsub.${ext}"`;
