...
async function dailyMotionHandler2(bot, msg, value, config) {
    ...
    if(!value) return bot.sendMessage(msg.chat.id, 'Silakan masukkan judul video, contoh: /dm2 <judul>'); 
    ...
    let result = await dailyMotionUpload({filePath: info.path, title: value, channelId: 'x3qdbhk', isCreatedForKids: false});
...
async function dailyMotionUpload(opts) {
    ...
    const clientId = config.DM_APIKEY2;
    const clientSecret = config.DM_APISECRET2;
